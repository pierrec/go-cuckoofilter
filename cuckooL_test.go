...
var (
	fullFilter64L *cuckoofilter.Filter64L
	fullFilterL   *cuckoofilter.FilterL
)

func initFilterL(wg *sync.WaitGroup) {
	j, n := 0, len(data)
	fullFilter64L = cuckoofilter.New64L(uint(n))
	fullFilterL = cuckoofilter.NewL(uint(n))
	for i := 0; i < n; i++ {
		fullFilterL.Insert(data[j])
		fullFilter64L.Insert(data64[j])
		if j++; j == n {
			j = 0
		}
	}
...
func TestLenCapL(t *testing.T) {
	f := cuckoofilter.New64L(100)
	if n := f.Len(); n != 0 {
...
func TestInsertHasL(t *testing.T) {
	f := cuckoofilter.New64L(128)
...
func TestDeleteHasL(t *testing.T) {
	f := cuckoofilter.New64L(128)
...
func TestFilterL(t *testing.T) {
	f := cuckoofilter.NewL(uint(len(data)))
	...
	//fmt.Printf("L: ok=%d err=%d errRate=%.5f%%\n", success, errNum, errRate)
	if errRate > 0.005 {
