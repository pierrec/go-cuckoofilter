...
var (
	fullFilter64M *cuckoofilter.Filter64M
	fullFilterM   *cuckoofilter.FilterM
)

func initFilterM(wg *sync.WaitGroup) {
	j, n := 0, len(data)
	fullFilter64M = cuckoofilter.New64M(uint(n))
	fullFilterM = cuckoofilter.NewM(uint(n))
	for i := 0; i < n; i++ {
		fullFilterM.Insert(data[j])
		fullFilter64M.Insert(data64[j])
		if j++; j == n {
			j = 0
		}
	}
...
func TestLenCapM(t *testing.T) {
	f := cuckoofilter.New64M(100)
	if n := f.Len(); n != 0 {
...
func TestInsertHasM(t *testing.T) {
	f := cuckoofilter.New64M(128)
...
func TestDeleteHasM(t *testing.T) {
	f := cuckoofilter.New64M(128)
...
func TestFilterM(t *testing.T) {
	f := cuckoofilter.NewM(uint(len(data)))
	...
	//fmt.Printf("M: ok=%d err=%d errRate=%.5f%%\n", success, errNum, errRate)
	if errRate > 1 {
